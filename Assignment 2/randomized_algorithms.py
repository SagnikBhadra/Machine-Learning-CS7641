...
def print_optimal_info_rhc(randomized_hill_climb_graph):
    optimal_index = randomized_hill_climb_graph.Fitness.idxmax()
...
def performance(info):
    ...
    print(f'The max value of the fitness function was: {last_iters.Fitness.max()}')
...
def randomized_hill_climb(problem):
    ...
    c_randomized_hill_climb = mlrose_hiive.RHCRunner(problem, experiment_name="first_try", iteration_list=[10000], seed=42, max_attempts=100, restart_list=[100])
...
def simulated_annealing(problem, temperature_list):
    print('\nSimulated Annealing')
    start = timeit.default_timer()
    c_simulated_annealing = mlrose_hiive.SARunner(problem, experiment_name="SA_Exp", iteration_list=[100000], temperature_list= temperature_list, decay_list=[mlrose_hiive.ExpDecay, mlrose_hiive.GeomDecay], seed=42, max_attempts=100)
...
def genetic_algorithm(problem, population_sizes, mutation_rates):
    print('\nGenetic Algorithm')
    start = timeit.default_timer()
    genetic_algorithm = mlrose_hiive.GARunner(problem=problem, experiment_name="GA_Exp", seed=42, iteration_list=[100000], max_attempts=100, population_sizes=population_sizes, mutation_rates=mutation_rates)
    genetic_algorithm_info, genetic_algorithm_graph = genetic_algorithm.run()
...
def mimic(problem, population_sizes, keep_percent_list):
    print('\nMIMIC')
    start = timeit.default_timer()
    c_mimic = mlrose_hiive.MIMICRunner(problem=problem, experiment_name="MMC_Exp", seed=42, iteration_list=[100000], max_attempts=100, population_sizes=population_sizes, keep_percent_list=keep_percent_list, use_fast_mimic=True)
